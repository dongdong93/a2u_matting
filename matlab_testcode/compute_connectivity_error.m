...
%     if thresh_steps(ii) == 1
%         thresh_steps(ii) = 0.98;
%     end
...
loss = sum(sum(abs(pred_phi - target_phi).*single(trimap==128)));   %1
% loss = sum(sum(abs(pred_phi - target_phi).*single(trimap==128))) / sum(sum(single(trimap==128)));   %2
% loss = sum(sum(abs(pred_phi - target_phi))) / 480000;   %3
