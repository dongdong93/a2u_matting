...
// this loop handles the case when having more threads than GPU actually supports
...
__global__ void BidownsampleNAIVEForward(const int nthreads,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    int up_pw = pw * scale_factor;
    int up_ph = ph * scale_factor;
    int up_width = width * scale_factor;
    int up_height = height * scale_factor;
    int start_w = (kernel_size % 2 ==0) ? up_pw - (kernel_size - 2) / 2 : up_pw - (kernel_size - 1) / 2;
    int end_w =  (kernel_size % 2 ==0) ? up_pw + kernel_size / 2 + 1 : up_pw + (kernel_size - 1) / 2 + 1;
    int start_h = (kernel_size % 2 ==0) ? up_ph - (kernel_size - 2) / 2 : up_ph - (kernel_size - 1) / 2;
    int end_h = (kernel_size % 2 ==0) ? up_ph + kernel_size / 2 + 1 : up_ph + (kernel_size - 1) / 2 + 1;

    scalar_t output_val = 0;
    for (int iy = start_h; iy < end_h; iy++) {
      for (int ix = start_w; ix < end_w; ix++) {
        if (iy < 0 || iy > up_height - 1 || ix < 0 || ix > up_width - 1) {
          continue;
        }
        int mask_iy = (kernel_size % 2 ==0) ? iy - up_ph + (kernel_size - 2) / 2 : iy - up_ph + (kernel_size - 1) / 2;
        int mask_ix = (kernel_size % 2 ==0) ? ix - up_pw + (kernel_size - 2) / 2 : ix - up_pw + (kernel_size - 1) / 2;
        int mask_c =
            (mask_group * kernel_size + mask_iy) * kernel_size + mask_ix;
        int feat_index =
            Loc2Index(n, c, iy, ix, channels, up_height, up_width);
        int mask_index =
...
int BidownsampleNAIVEForwardLaucher(const at::Tensor features, const at::Tensor masks,
                              const int kernel_size, const int group_size,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      features.scalar_type(), "BidownsampleNAIVELaucherForward", ([&] {
        const scalar_t *bottom_data = features.data_ptr<scalar_t>();
        const scalar_t *bottom_masks = masks.data_ptr<scalar_t>();
        scalar_t *top_data = output.data_ptr<scalar_t>();

        BidownsampleNAIVEForward<scalar_t>
...
template <typename scalar_t>
__global__ void BidownsampleNAIVEBackward(
...
int BidownsampleNAIVEBackwardLaucher(const at::Tensor top_grad,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      top_grad.scalar_type(), "BidownsampleNAIVELaucherBackward", ([&] {
        ...
        BidownsampleNAIVEBackward<scalar_t>
            <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
